...
<head class="dark">
    ...
    <meta property="og:site_name" content="Cbcontrol">
    <meta property="og:title"
        content="@if ($company->corporate_reason != null) {{ $company->corporate_reason }} @else Cbcontrol Plataform @endif" />
    <meta property="og:description" content="Plataforma de Gestão e delivry, densenvolvida pensando em você !" />
    <meta property="og:image" itemprop="image"
        content="
        @if ($company->image()->value('path') != null) {{ url(Storage::url($company->image()->value('path'))) }}
        @else 
        https://i.postimg.cc/wv0DWcx8/Group-1.png @endif
        ">
    <meta property="og:image"
        content=" @if ($company->image()->value('path') != null) {{ url(Storage::url($company->image()->value('path'))) }}
@else 
https://i.postimg.cc/wv0DWcx8/Group-1.png @endif">
...
<body class="font-sans antialiased">
    <header
        class="header w-full h-[450px] sm:flex sm:items-center flex items-center border border-b-3 bg-white bg-cover bg-no-repeat bg-center"
        @if ($company->image()->value('path') != null) style="background-image: url('{{ url(Storage::url($company->image()->value('path'))) }}')" @else style="background-image: url('/img/logo/wallpaper-cb.png')" @endif>
        <div class="w-full h-full mx-auto flex justify-center items-center"
            ...
            <div class="conten-ifon-company flex flex-col items-center">
                ...
                <img alt=""
                    @if ($company->image()->value('path') != null) src="{{ url(Storage::url($company->image()->value('path'))) }}" @else src="/img/logo/cb-logo.png" @endif
                    width="150px" height="150px" class="rounded-full">
    ...
    <section class="bar-alert w-full h-[50px] shadow-lg">
        <div class="container mx-auto h-[50px] flex items-center justify-between font-bold text-gray-600">
            ...
            <div class="content-alert-bar-menu flex ">
                @if (!Auth::guest())
                    <div class="btn-group ">
                        <button type="button" class="dropdown-toggle flex items-center mr-3" data-toggle="dropdown"
                    ...
                    @else
                        <a href="#" class="show-modal-login-user mr-2" type="button"
        ...
        @if (!$company->status)
            <div class=" mt-4">
                <div id="alert-additional-content-2"
                    class="p-4 mb-4 border border-red-300 rounded-lg bg-red-50 dark:bg-red-200" role="alert">
                    ...
                    <div class="mt-2 mb-4 text-sm text-red-900 dark:text-red-800">
                        Essa empresa em encontrasse inativa, não será possível adicionar itens ao carrinho ou finalizar
                        comprar.
                    </div>
                </div>
            </div>
        ...
        @if (!$company->settings->is_opened)
            <div class=" mt-4">
                <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4" role="alert">
                    ...
                    <p>Essa loja encorou o expediente por hoje, mas fique tranquilo os seus pedidos serão salvos e
                        poderão ser
        ...
        @if (!$company->settings->has_delivery)
            <div class=" mt-4">
                <div class="bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3" role="alert">
                    ...
                    <p class="text-sm">Desculpe mas não estamos com o delivery disponível hoje, somente retirada na
                        nossa loja
                    </p>
                </div>
            </div>
    ...
    <div class="w-full h-[100px] sm:h-[50px] shadow-lg fixed bottom-[-2px] z-1">
        <section class="bg-orange-600 h-12 ">
            <div class="container mx-auto flex  justify-between ">
                <article class="h-12 flex items-center justify-center font-bold text-white cursor-pointer"
                    onclick="Livewire.emit('openModal', 'store.cart.modal-cart-itens')">
    ...
    <footer class="w-full h-full bg-black flex  justify-center items-center">
        <div class="container mx-auto flex flex-col justify-center items-center">
            <div
                class="w-full footer-descritpion text-white font-bold text-xl sm:text-2xl text-left mb-[30px] font-mono">
                <div class="information-company w-full text-center text-gray-500 mt-4">
                    ...
                    <p class="text-sm w-full flex items-center justify-center mt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                            class="bi bi-pin-map-fill mr-2" viewBox="0 0 16 16">
                            <path fill-rule="evenodd"
                                d="M3.1 11.2a.5.5 0 0 1 .4-.2H6a.5.5 0 0 1 0 1H3.75L1.5 15h13l-2.25-3H10a.5.5 0 0 1 0-1h2.5a.5.5 0 0 1 .4.2l3 4a.5.5 0 0 1-.4.8H.5a.5.5 0 0 1-.4-.8l3-4z" />
                            <path fill-rule="evenodd"
                                d="M4 4a4 4 0 1 1 4.5 3.969V13.5a.5.5 0 0 1-1 0V7.97A4 4 0 0 1 4 3.999z" />
                ...
                <div
                    class="social-media-company w-full text-center mt-2 text-gray-500 flex items-center justify-center">
                    ...
                    <a href="https://api.whatsapp.com/send?phone=7398272867&text=Óla ! Gostaria de conhecer um pouco mais da plataforma, vi a pagina da {{ $company->corporate_reason }} e fiquei muito interessado."
                        target="_blank">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
            ...
            <div class="w-full footer-more-info border-dashed border-t-2 border-orange-600 mb-[30px]">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-500 mt-[30px]">
                    <p class="text-center sm:text-left text-xs">CbSoftWare<a href="#"> Termos e Condições</a>
                    </p>
